...
public struct UIScheduler: Scheduler {
  ...
  public typealias SchedulerTimeType = CXWrappers.DispatchQueue.SchedulerTimeType
  ...
  public var now: SchedulerTimeType { DispatchQueue.main.cx.now }
  public var minimumTolerance: SchedulerTimeType.Stride { DispatchQueue.main.cx.minimumTolerance }

  public func schedule(options: SchedulerOptions? = nil, _ action: @escaping () -> Void) {
    if DispatchQueue.getSpecific(key: key) == value {
      action()
    } else {
      DispatchQueue.main.cx.schedule(action)
    }
  }
  ...
  public func schedule(
    ...
    DispatchQueue.main.cx.schedule(after: date, tolerance: tolerance, options: nil, action)
  }
